...
def updateApplication(Map config) {
    ...
    try {
        ...
        sh """
            ...
            docker build -t ${env.ECR_REPO_NAME}:app_1 --build-arg APP_NAME=1 .
            docker tag ${env.ECR_REPO_NAME}:app_1 ${ecrUri}:app_1
        ...
        sh """
            aws ecr get-login-password --region ${env.AWS_REGION} | docker login --username AWS --password-stdin ${ecrUri}
            cd ${env.WORKSPACE}/modules/ecs/scripts
            docker build -t ${env.ECR_REPO_NAME}:app_2 --build-arg APP_NAME=2 .
            docker tag ${env.ECR_REPO_NAME}:app_2 ${ecrUri}:app_2
        ...
        sh """
            aws ecr get-login-password --region ${env.AWS_REGION} | docker login --username AWS --password-stdin ${ecrUri}
            cd ${env.WORKSPACE}/modules/ecs/scripts
            docker build -t ${env.ECR_REPO_NAME}:app_3 --build-arg APP_NAME=3 .
            docker tag ${env.ECR_REPO_NAME}:app_3 ${ecrUri}:app_3
