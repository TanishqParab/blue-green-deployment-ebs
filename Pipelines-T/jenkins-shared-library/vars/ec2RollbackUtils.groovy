...
def prepareRollback(Map config) {
    ...
    if (!blueLine) {
        error "❌ No instance with tag Name=${blueInstanceTag} found in green-tg. Cannot proceed with rollback."
    }
    ...
    if (!healthState) {
        error "❌ ${blueInstanceTag} is not currently registered in green-tg or health data is missing."
    }
