...
def deployToBlueService(Map config) {
    ...
    try {
        ...
        // Build and push Docker image for all apps
        // App 1
        sh """
            ...
            docker build -t ${config.ecrRepoName}:app_1 --build-arg APP_NAME=1 .
            docker tag ${config.ecrRepoName}:app_1 ${ecrUri}:app_1
            docker push ${ecrUri}:app_1
        """
        ...
        sh """
            aws ecr get-login-password --region ${config.awsRegion} | docker login --username AWS --password-stdin ${ecrUri}
            cd ${config.tfWorkingDir}/modules/ecs/scripts
            docker build -t ${config.ecrRepoName}:app_2 --build-arg APP_NAME=2 .
            docker tag ${config.ecrRepoName}:app_2 ${ecrUri}:app_2
            docker push ${ecrUri}:app_2
        """
        ...
        sh """
            aws ecr get-login-password --region ${config.awsRegion} | docker login --username AWS --password-stdin ${ecrUri}
            cd ${config.tfWorkingDir}/modules/ecs/scripts
            docker build -t ${config.ecrRepoName}:app_3 --build-arg APP_NAME=3 .
            docker tag ${config.ecrRepoName}:app_3 ${ecrUri}:app_3
            docker push ${ecrUri}:app_3
        """
