        ...
        if (config.implementation == 'ec2') {
            ...
            instanceList.each { instance ->
                ...
                sshagent([config.sshKeyId]) {
                    sh """
                    # Determine app file based on app name
                    APP_FILE="${config.appName ? "app_${config.appName}.py" : config.appFile}"
                    scp -o StrictHostKeyChecking=no ${config.tfWorkingDir}/modules/ec2/scripts/$APP_FILE ec2-user@${instance}:/home/ec2-user/$APP_FILE
                    ...
                    ssh ec2-user@${instance} "chmod +x /home/ec2-user/setup_flask_service.py && sudo python3 /home/ec2-user/setup_flask_service.py ${config.appName ?: ""}"
                    """
                }
            }
